...
public class ConcurrentLRUHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>,
            ...
            if (count != 0) { // read-volatile
                ...
                while (e != null) {
                    if (e.hash == hash && key.equals(e.key)) {
                        V v = e.value;
                        if (v != null) return readValueUnderLock(e); // recheck
                        return v;
                    }
                    e = e.next;
                }
    ...
    private ConcurrentLRUHashMap(int maxSize, int concurrencyLevel, boolean debug) {
        if (maxSize < 0 || concurrencyLevel <= 0) throw new IllegalArgumentException();
    ...
    public ConcurrentLRUHashMap(int maxSize, int concurrencyLevel) {
        this(maxSize, concurrencyLevel, false);
    }
    ...
    public static void main(String[] args) {
        ...
        ConcurrentLRUHashMap<String, String> m = new ConcurrentLRUHashMap<String, String>(20);
        for (int i = 1; i <= 180; i++) {
            try {
                Thread.sleep(80);
...
//        LinkedHashMap<String,String> link = new LinkedHashMap<String, String>(20);
//        for (int i = 1; i <= 80; i++) {
//            link.put(i + "", i + "");
//        }
